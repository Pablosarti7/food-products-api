...
def add_product(item: FoodProduct, session: Session = Depends(get_session), api_key: str = Depends(verify_api_key)):
    ...
    # Fetches the first matching result or None if no match found.
    existing_product = results.first()

    if existing_product:
        raise HTTPException(
            status_code=409, detail="Product with the same name already exists.")
...
def get_ingredient(product_name: str, session: Session = Depends(get_session)):
    statement = select(FoodProduct).where(
        FoodProduct.name.contains(product_name))
